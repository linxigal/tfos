...
class TFOS(object):
    def __init__(self, sc, input_mode=TFCluster.InputMode.SPARK):
        self.sc = sc
    ...
    def train(self, data_rdd, model_rdd, cluster_size, num_ps, batch_size, epochs, model_dir):
        ...
        cluster = TFCluster.run(self.sc, worker, None, cluster_size, num_ps, input_mode=self.input_mode)
        cluster.train(data_rdd.rdd)

    @ext_exception('inference model')
    def inference(self, data_rdd, model_rdd, cluster_size, num_ps, model_dir):
        n_samples = data_rdd.count()
