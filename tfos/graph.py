...
GRAPH = namedtuple("graph", ['label', 'prediction', 'accuracy', 'loss', 'saver', 'summary_op', 'init_op'])
...
class Worker(object):
    ...
    def rdd_data(self):
        ...
        ds = tf.data.Dataset.from_generator(rdd_generator, (tf.float32, tf.float32),
                                            (tf.TensorShape([4]), tf.TensorShape([1]))).batch(self.args.batch_size)
    ...
    def worker(self, cluster, task_index):
        with tf.device(
            ...
            x, y = self.rdd_data()
            pred, loss, train_op = self.main_func(x, y)
            accuracy = None
            ...
            self.graph = GRAPH._make([y, pred, accuracy, loss, saver, summary_op, init_op])
    ...
    def start_session(self, worker_num, task_index, master):
        ...
        with tf.train.MonitoredTrainingSession(master=master,
            ...
            while not sess.should_stop() and not self.tf_feed.should_stop():
                if self.mode == "train":
                    _, summary, step = sess.run([self.graph.train_op, g.summary_op, g.global_step])
                    if (step % 2 == 0) and (not sess.should_stop()):
                        # print("{} step: {} accuracy: {}".format(datetime.now().isoformat(), step, sess.run(accuracy)))
                        if self.model_type == 0:
                            logging.error(
                                "{} step: {} accuracy: {}".format(datetime.now().isoformat(), step,
                                                                  sess.run(g.accuracy)))
                        else:
                            logging.error(
                                "{} step: {} loss: {}".format(datetime.now().isoformat(), step, sess.run(g.loss)))
                    if task_index == 0:
                        summary_writer.add_summary(summary, step)
                else:  # args.mode == "inference"
                    labels, preds, acc = sess.run([g.label, g.prediction, g.accuracy])
        ...
        if sess.should_stop() or step >= self.args.steps:
