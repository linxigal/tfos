...
class KWorker(Worker):

    def __init__(self, *args, **kwargs):
        super(KWorker, self).__init__(*args, **kwargs)
    ...
    def model_tmp_path(self):
        return os.path.join(self.tmp_dir, '{}.{}'.format(self.name, self.model_suffix))
...
class TrainWorker(KWorker):
    ...
    def execute(self):
        ...
        with tf.Session(self.server.target, config=config) as sess:
            ...
            ckpt_callback = ModelCheckpoint(self.checkpoint_path, monitor='loss', save_weights_only=True)
